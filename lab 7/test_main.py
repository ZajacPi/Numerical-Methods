...
results_is_nonsingular = expected['is_nonsingular'] 
...
@pytest.mark.parametrize("A, result", results_is_nonsingular)
def test_is_nonsingular(A: np.ndarray, result):
    if result is None:
        assert main.is_nonsingular(A) is None, 'Spodziewany wynik: {0}, aktualny {1}. Błedy wejścia.'.format(result,
                                                                                                      main.is_nonsingular(A))
    else:
        tresult = main.is_nonsingular(A)
        assert tresult == result, 'Spodziewany wynik: {0}, aktualny {1}. Błedy wejścia.'.format(result, main.is_nonsingular(A))
