...
def random_matrix_Ab(m:int):
    ...
    if not isinstance(m, int) or m<0:
        return None
    A_matrix = np.random.randint(0, 100, size = (m, m))
    # print(A_matrix)
    B_vector = np.random.randint(0, 100, size = m)
...
def log_sing_value(n:int, min_order:Union[int,float], max_order:Union[int,float]):
    ...
    # if min_order >= max_order or n <= 0 or not isinstance(n, int) or (not isinstance(max_order, Union[int,float])):
    #     return None
    # log_s_val = np.logspace(min_order, max_order, num=n)
    # return log_s_val[::-1]
    try: 
...
def order_sing_value(n:int, order:Union[int,float] = 2, site:str = 'gre'):
    """Funkcja generująca wektor losowych wartości singularnych (n,) będących wartościami zmiennoprzecinkowymi losowanymi przy użyciu funkcji np.random.rand(n)*10. 
        ...
        """
    try:
        if n <= 0 or site not in ['low', 'gre']:
                return None
        
        singular_value = np.sort(np.random.rand(n)*10)

        if site == 'low':
            singular_value[-1] *= 10 ** order
        elif site == 'gre':
            singular_value[0] *= 10 ** order
        singular_value = np.sort(singular_value)[::-1]
